...
'(--app --lib --package --build-backend --description)--script[Create a script]' \
'(--script)--no-description[Disable the description for the project]' \
...
'--output-format=[The format of the output]:OUTPUT_FORMAT:((text\:"Display the version as plain text"
...
'(--all-packages --package --no-install-project --no-install-workspace --extra --all-extras --no-extra --no-all-extras --dev --no-dev --only-dev --group --no-group --no-default-groups --only-group --all-groups)--script=[Sync the environment for a Python script, rather than the current project]:SCRIPT:_files' \
...
(lock)
_arguments "${_arguments_options[@]}" : \
'--script=[Lock the specified Python script, rather than the current project]:SCRIPT:_files' \
'*--index=[The URLs to use when resolving dependencies, in addition to the default index]:INDEX:_default' \
'--default-index=[The URL of the default package index (by default\: <https\://pypi.org/simple>)]:DEFAULT_INDEX:_default' \
'-i+[(Deprecated\: use \`--default-index\` instead) The URL of the Python package index (by default\: <https\://pypi.org/simple>)]:INDEX_URL:_default' \
'--index-url=[(Deprecated\: use \`--default-index\` instead) The URL of the Python package index (by default\: <https\://pypi.org/simple>)]:INDEX_URL:_default' \
...
'*--with-requirements=[Include all requirements listed in the given \`requirements.txt\` files]:WITH_REQUIREMENTS:_default' \
...
'(--only-downloads)--only-installed[Only show installed Python versions]' \
'(--only-installed)--only-downloads[Only show available Python downloads]' \
...
'-h[Display the concise help for this command]' \
'--help[Display the concise help for this command]' \
'::request -- A Python request to filter by:_default' \
&& ret=0
;;
(install)
_arguments "${_arguments_options[@]}" : \
...
'-h[Display the concise help for this command]' \
'--help[Display the concise help for this command]' \
'*::targets -- The Python minor version(s) to upgrade:_default' \
&& ret=0
;;
(find)
_arguments "${_arguments_options[@]}" : \
...
(install)
_arguments "${_arguments_options[@]}" : \
'*-r+[Install all packages listed in the given \`requirements.txt\` or \`pylock.toml\` files]:REQUIREMENTS:_files' \
'*--requirements=[Install all packages listed in the given \`requirements.txt\` or \`pylock.toml\` files]:REQUIREMENTS:_files' \
...
'upgrade:Upgrade installed Python versions to the latest supported patch release (requires the \`--preview\` flag)' \
